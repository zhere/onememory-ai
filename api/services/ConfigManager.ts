...
export interface BaseSegmentationConfig {
...
export interface ModuleSpecificSegmentationConfig {
...
export interface SegmentationConfig {
  base: BaseSegmentationConfig;
  modules: ModuleSpecificSegmentationConfig;
  scenarioDefaults: {
...
export class ConfigManager {
  ...
  private initializeDefaults(): void {
    ...
    const baseSegmentation: BaseSegmentationConfig = {
      strategy: 'semantic',
      chunkSize: 1000,
      overlap: 200,
      separators: ['\n\n', '\n', '. ', '! ', '? '],
   ...
   */
  async updateSegmentationConfig(
    configUpdate: Partial<{
      base?: Partial<BaseSegmentationConfig>;
      modules?: Partial<ModuleSpecificSegmentationConfig>;
      scenarioDefaults?: Partial<SegmentationConfig['scenarioDefaults']>;
    }>,
    ...
    try {
      ...
      // Create updated config with deep merging
      const updatedConfig: SegmentationConfig = {
        ...
        modules: {
          ...
          ...configUpdate.modules,
          memory: {
            ...currentConfig.modules.memory,
            ...(configUpdate.modules?.memory || {}),
          },
          rag: {
            ...currentConfig.modules.rag,
            ...(configUpdate.modules?.rag || {}),
          },
          context: {
            ...currentConfig.modules.context,
            ...(configUpdate.modules?.context || {}),
          },
          token: {
            ...currentConfig.modules.token,
            ...(configUpdate.modules?.token || {}),
          },
        },
        scenarioDefaults: {
          ...currentConfig.scenarioDefaults,
          ...configUpdate.scenarioDefaults,
          longDocument: {
            ...currentConfig.scenarioDefaults.longDocument,
            ...(configUpdate.scenarioDefaults?.longDocument || {}),
          },
          shortConversation: {
            ...currentConfig.scenarioDefaults.shortConversation,
            ...(configUpdate.scenarioDefaults?.shortConversation || {}),
          },
          codeSnippets: {
            ...currentConfig.scenarioDefaults.codeSnippets,
            ...(configUpdate.scenarioDefaults?.codeSnippets || {}),
          },
        },
      };
  ...
  private validateBaseSegmentationConfig(config: any, result: ValidationResult, context: string): void {
    const validStrategies = ['fixed', 'semantic', 'adaptive', 'hybrid'];
    if (!validStrategies.includes(config.strategy)) {
      result.errors.push(`${context}: Segmentation strategy must be one of: ${validStrategies.join(', ')}`);
    }
    if (config.chunkSize <= 0) {
      result.errors.push(`${context}: Chunk size must be positive`);
    }
    if (config.overlap < 0 || config.overlap >= config.chunkSize) {
      result.errors.push(`${context}: Overlap must be non-negative and less than chunk size`);
    }
    if (config.minChunkSize <= 0 || config.minChunkSize > config.maxChunkSize) {
      result.errors.push(`${context}: Min chunk size must be positive and less than max chunk size`);
    }
    if (config.semanticThreshold < 0 || config.semanticThreshold > 1) {
      result.errors.push(`${context}: Semantic threshold must be between 0 and 1`);
    }
  }
  ...
  private validateSegmentationConfig(config: any, result: ValidationResult): void {
    ...
    if (config.base) {
      this.validateBaseSegmentationConfig(config.base, result, 'Base segmentation config');
    } else {
      result.errors.push('Base segmentation config is required');
    }
    ...
    if (config.modules) {
      for (const [moduleName, moduleConfig] of Object.entries(config.modules)) {
        if (moduleConfig) {
          this.validateBaseSegmentationConfig(moduleConfig, result, `${moduleName} module config`);
        }
      }
    }

    // Validate scenario defaults
    if (config.scenarioDefaults) {
      for (const [scenarioName, scenarioConfig] of Object.entries(config.scenarioDefaults)) {
        if (scenarioConfig) {
          this.validateBaseSegmentationConfig(scenarioConfig, result, `${scenarioName} scenario config`);
        }
      }
    }
  }
