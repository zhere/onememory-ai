...
export class DatabaseService {
  ...
  async createTokenConfig(tokenConfig: Omit<TokenConfig, 'id' | 'createdAt' | 'updatedAt'>): Promise<TokenConfig> {
    const query = `
      INSERT INTO token_configs (project_id, max_context_tokens, reserved_tokens, compression_config, priority_weights, auto_optimization)
      VALUES ($1, $2, $3, $4, $5, $6)
      ON CONFLICT (project_id) DO UPDATE
      SET max_context_tokens = $2, 
          reserved_tokens = $3, 
          compression_config = $4, 
          priority_weights = $5, 
          auto_optimization = $6,
          updated_at = NOW()
      RETURNING *
    ...
    const values = [
      ...
      tokenConfig.maxContextTokens,
      tokenConfig.reservedTokens,
      JSON.stringify(tokenConfig.compressionConfig),
      JSON.stringify(tokenConfig.priorityWeights),
      tokenConfig.autoOptimization,
    ];

    const result = await this.query<any>(query, values);
    return this.mapTokenConfigRow(result[0]);
  }

  async getTokenConfigByProjectId(projectId: string): Promise<TokenConfig | null> {
    const query = 'SELECT * FROM token_configs WHERE project_id = $1';
    const result = await this.query<any>(query, [projectId]);
    return result.length > 0 ? this.mapTokenConfigRow(result[0]) : null;
  }
  ...
  async updateTokenConfig(id: string, updates: Partial<TokenConfig>): Promise<TokenConfig | null> {
    ...
    for (const [key, value] of Object.entries(updates)) {
      if (value !== undefined && key !== 'id' && key !== 'createdAt') {
        if (key === 'compressionConfig' || key === 'priorityWeights') {
          fields.push(`${this.camelToSnake(key)} = $${paramIndex}`);
          values.push(JSON.stringify(value));
        } else {
          fields.push(`${this.camelToSnake(key)} = $${paramIndex}`);
  ...
  async deleteTokenConfig(id: string): Promise<boolean> {
    const query = 'DELETE FROM token_configs WHERE id = $1';
    const result = await this.query(query, [id]);
    return result.length > 0;
  }
  ...
  private mapTokenConfigRow(row: any): TokenConfig {
    return {
      id: row.id,
      projectId: row.project_id,
      maxContextTokens: row.max_context_tokens,
      reservedTokens: row.reserved_tokens,
      compressionConfig: JSON.parse(row.compression_config || '{}'),
      priorityWeights: JSON.parse(row.priority_weights || '{}'),
      autoOptimization: row.auto_optimization,
      createdAt: new Date(row.created_at),
      updatedAt: new Date(row.updated_at),
    };
  }
